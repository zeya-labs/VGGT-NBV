...
- Generated artifacts: Lightning checkpoints and TensorBoard events live under `runs/` (Hydra `output_dir`/`log_dir`), with scratch outputs in `outputs/` or `TEMP/`. Keep large files out of git.
...
- Visualize logs: `tensorboard --logdir "runs/%Y-%m-%d/%H-%M-%S/logs/events/version_0"`.
...
- PRs should briefly state the experiment/config context, list any new Hydra parameters, and attach sample log metrics or screenshots (TensorBoard scalars) when changing training behavior.
